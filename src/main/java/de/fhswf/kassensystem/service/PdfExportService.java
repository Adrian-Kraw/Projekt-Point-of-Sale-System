...
public class PdfExportService {
    ...
    public byte[] exportiereTagebericht(TagesabschlussDTO tagesabschluss,
        ...
        java.math.BigDecimal bonWert = tagesabschluss.getAnzahlTransaktionen() > 0
                ? tagesabschluss.getGesamtumsatz().divide(
                java.math.BigDecimal.valueOf(tagesabschluss.getAnzahlTransaktionen()),
        ...
        java.math.BigDecimal gesamt = tagesabschluss.getGesamtumsatz();
        if (gesamt == null) gesamt = java.math.BigDecimal.ZERO;

        java.math.BigDecimal bar   = tagesabschluss.getUmsatzBar()   != null ? tagesabschluss.getUmsatzBar()   : java.math.BigDecimal.ZERO;
        java.math.BigDecimal karte = tagesabschluss.getUmsatzKarte() != null ? tagesabschluss.getUmsatzKarte() : java.math.BigDecimal.ZERO;

        String anteilBar   = gesamt.compareTo(java.math.BigDecimal.ZERO) > 0
                ? bar.multiply(java.math.BigDecimal.valueOf(100)).divide(gesamt, 1, java.math.RoundingMode.HALF_UP) + " %"
                : "0 %";
        String anteilKarte = gesamt.compareTo(java.math.BigDecimal.ZERO) > 0
                ? karte.multiply(java.math.BigDecimal.valueOf(100)).divide(gesamt, 1, java.math.RoundingMode.HALF_UP) + " %"
                : "0 %";
    ...
    private Cell tabellenHeader(String text, PdfFont font) {
        return new Cell()
                ...
                .setBorderBottom(new com.itextpdf.layout.borders.SolidBorder(BRAUN, 1))
                .setPadding(8);
    }
    ...
    private String formatBetrag(java.math.BigDecimal betrag) {
     ...
     *
     * @param positionen  Verkaufspositionen aus dem Warenkorb
     * @param rabattProzent Rabatt in Prozent (0 = kein Rabatt)
     * @return PDF als Byte-Array
     */
    public byte[] exportiereKassenbon(
            java.util.List<de.fhswf.kassensystem.model.Verkaufsposition> positionen,
            java.math.BigDecimal rabattProzent) throws IOException {
        ...
        doc.add(new Paragraph("Canapé Café")
                ...
                .setTextAlignment(TextAlignment.CENTER));
        doc.add(new Paragraph(java.time.LocalDateTime.now()
        ...
        // Positionen
        java.math.BigDecimal zwischensumme = java.math.BigDecimal.ZERO;
        java.math.BigDecimal mwst7  = java.math.BigDecimal.ZERO;
        java.math.BigDecimal mwst19 = java.math.BigDecimal.ZERO;

        for (de.fhswf.kassensystem.model.Verkaufsposition pos : positionen) {
            java.math.BigDecimal posGesamt = pos.getEinzelpreis()
                    .multiply(java.math.BigDecimal.valueOf(pos.getMenge()));
            zwischensumme = zwischensumme.add(posGesamt);

            // MwSt aufschlüsseln
            if (pos.getArtikel().getMehrwertsteuer() != null) {
                java.math.BigDecimal satz = pos.getArtikel().getMehrwertsteuer().getSatz();
                java.math.BigDecimal nettoAnteil = posGesamt.divide(
                        java.math.BigDecimal.ONE.add(satz.divide(java.math.BigDecimal.valueOf(100))),
                        2, java.math.RoundingMode.HALF_UP);
                java.math.BigDecimal steuerBetrag = posGesamt.subtract(nettoAnteil);
                if (satz.compareTo(java.math.BigDecimal.valueOf(7)) == 0) {
        ...
        java.math.BigDecimal rabattBetrag = java.math.BigDecimal.ZERO;
        if (rabattProzent != null && rabattProzent.compareTo(java.math.BigDecimal.ZERO) > 0) {
        ...
        java.math.BigDecimal gesamtsumme = zwischensumme.subtract(rabattBetrag);

        // MwSt
        if (mwst7.compareTo(java.math.BigDecimal.ZERO) > 0) {
            addBonZeile(doc, "MwSt 7%", formatBetrag(mwst7), fontNormal, false);
        }
        if (mwst19.compareTo(java.math.BigDecimal.ZERO) > 0) {
